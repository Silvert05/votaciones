...
@Component({
  selector: 'admin-resultados-finales',
  imports: [
    ReactiveFormsModule,
    MatButtonModule,
    MatIconModule,
...
export default class ResultadosFinalesComponent implements OnInit {
  ...
  loadResumen(): void {
    ...
    this._escrutinioService
      ...
      .subscribe({
        ...
        error: (err) =>
          this._notify(
            this.errorMessage(err, 'No se pudieron cargar resultados finales.'),
          ),
      });
  }
